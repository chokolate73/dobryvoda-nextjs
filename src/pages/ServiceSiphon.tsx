...
import heroImage from "@/assets/work-4.jpg";
...
const ServiceSiphon = () => {
  ...
  return (
    <main className="min-h-screen">
      <SeoHead 
        title={t("siphon.seo.title")}
        description={t("siphon.seo.description")}
        keywords={t("siphon.seo.keywords")}
      />
      <Navbar />
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {[1, 2, 3, 4].map((num) => (
              <div key={num} className="flex items-start gap-4 p-6 bg-card rounded-xl border border-border hover:shadow-lg transition-all">
                ...
                <div>
                  <h3 className="font-semibold text-lg mb-1">{t(`siphon.problem.${num}`)}</h3>
                </div>
              </div>
            ))}
          </div>
